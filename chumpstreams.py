...
Version: 2.0.6
Author: covchump
Last updated: 2025-05-26 15:15:33
...
from PyQt5.QtWidgets import (
    QApplication, QMessageBox, QAction, QMenu, QListWidgetItem, 
...
# Constants
VERSION = "2.0.6"
SERVER = "covchump.manintheknow.xyz"
USE_HTTPS = True
...
# Monkey patch the ChumpStreamsMainWindow to use our improved login dialog
...
# Patch ContentPanel to enable right-click favorites
from PyQt5.QtWidgets import QMenu

# Store the original contextMenuEvent method
if hasattr(QListWidget, 'contextMenuEvent'):
    original_context_menu_event = QListWidget.contextMenuEvent

# Modified version that includes favorite toggle
def patched_context_menu_event(self, event):
    ...
    if not hasattr(self, 'parent') or not hasattr(self.parent(), 'create_content_menu'):
        # Not our content list, use original method
        original_context_menu_event(self, event)
        return
...
# This function will be called from ChumpStreamsApp to apply the patch
...
# New function to disable all context menus
...
# Function to remove favorite buttons but keep the Favorites tab
...
class ChumpStreamsApp:
    ...
    def __init__(self, app):
        ...
        # Show the window
        # The window will be shown by main() after splash screen
    ...
    def _load_config(self):
        ...
        try:
            ...
            if saved_credentials:
                ...
                if username:
                    self.window.set_login_credentials(username, True)
    ...
    def _auto_login(self):
        ...
        if self.auth_manager.has_saved_credentials():
            ...
            self._login(username, password, True)
        else:
    ...
    def _login(self, username, password, remember):
        """Handle login request"""
        self.window.show_status_message("Logging in...")
        
        # Call auth manager to handle login
        self.auth_manager.login(username, password, remember)
    ...
    def _play_content(self, item):
        ...
        result = self.playback_manager.play_content(item, self.content_type, self.window, original_type=self.content_type)
    ...
    try:
        ...
        def check_splash():
            if splash.isVisible():
                ...
                # Splash is gone, show main window
                show_main_window()
        
        # Start checking
        QTimer.singleShot(100, check_splash)
        
        # Handle application close to ensure VLC processes are killed
